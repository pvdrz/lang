...
pub(crate) fn pretty_print(expr: &Expr) -> impl fmt::Display + '_ {
    struct PrettyPrintable<'a>(&'a Expr);
    ...
    impl fmt::Display for PrettyPrintable<'_> {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            ...
            pp.visit_expr(self.0)
        }
    }

    PrettyPrintable(expr)
}
...
impl Visitor for PrettyPrinter<'_, '_> {
    ...
    fn visit_expr_let(&mut self, expr_let: &super::ExprLet) -> fmt::Result {
        ...
        let insert_newline = matches!(&*expr_let.rhs, Expr::Let(_) | Expr::If(_) | Expr::Case(_));
    ...
    fn visit_expr_group(&mut self, expr_group: &super::ExprGroup) -> fmt::Result {
        self.parenthesize(|this| super::visitor::visit_expr_group(this, expr_group))
    }
