use crate::ir::{
    BinOp, Expr, ExprApp, ExprBinary, ExprCase, ExprFn, ExprIf, ExprLet, ExprUnary, Ident, Literal,
    Pat, UnOp,
};
...
pub(crate) trait Visitor {
    ...
    fn visit_expr_fn(&mut self, expr_fn: &ExprFn) -> Result<(), Self::Err> {
        visit_expr_fn(self, expr_fn)
    }
...
pub(crate) fn visit_expr<V: Visitor + ?Sized>(v: &mut V, expr: &Expr) -> Result<(), V::Err> {
    match expr {
        ...
        Expr::Fn(expr_fn) => v.visit_expr_fn(expr_fn),
...
pub(crate) fn visit_expr_fn<V: Visitor + ?Sized>(
    v: &mut V,
    expr_fn: &ExprFn,
) -> Result<(), V::Err> {
